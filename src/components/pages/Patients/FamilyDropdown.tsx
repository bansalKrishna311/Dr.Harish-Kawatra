...
const FamilyDropdown: React.FC<FamilyDropdownProps> = ({ patient_id }) => {
  ...
  useEffect(() => {
    const fetchFamilies = async () => {
      try {
        const response = await axios.get('https://dr-harish-kawatra.onrender.com/api/v1/families'); // Update with your actual endpoint
  ...
  const handleFamilyChange = async (selectedOption: FamilyOption | null) => {
    ...
    if (selectedOption) {
      try {
        ...
        await axios.put(`https://dr-harish-kawatra.onrender.com/api/v1/patients/${patient_id}/family`, {
